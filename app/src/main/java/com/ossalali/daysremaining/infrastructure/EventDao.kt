...
@Dao
interface EventDao {
  @Upsert suspend fun upsertEvent(eventItem: EventItem)

  @Upsert suspend fun upsertEvents(eventItems: List<EventItem>)
  ...
  @Query("SELECT * FROM eventitem order by id desc") fun getAllEventsAsFlow(): Flow<List<EventItem>>

  @Query("SELECT * FROM eventitem order by id desc") suspend fun getAllEvents(): List<EventItem>
  ...
  @Query("SELECT * FROM eventitem ORDER BY id DESC LIMIT 1") suspend fun getFirstEvent(): EventItem

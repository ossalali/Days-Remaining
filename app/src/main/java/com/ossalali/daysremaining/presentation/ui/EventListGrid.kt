...
fun EventListGrid(
    ...
    onEventItemSelection: (EventItem) -> Unit,
    ...
    LazyVerticalGrid(columns = GridCells.Fixed(2), modifier = modifier) {
        items(events, key = { event -> event.id }) { event ->
            val isSelected = selectedEvents.map { it.id }.contains(event.id)
            Card(
                border =
                    if (isSelected) {
                        ...
                        .combinedClickable(
                            onClick = {
                                if (selectedEvents.isEmpty()) {
                                    ...
                                    onEventItemSelection(event)
                                }
                            },
                            onLongClickLabel = "Event Selected",
                            onLongClick = { onEventItemSelection(event) },
                        ),
